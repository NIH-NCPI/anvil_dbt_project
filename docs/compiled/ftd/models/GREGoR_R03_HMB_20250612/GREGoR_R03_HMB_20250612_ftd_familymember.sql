    ...
    with source as (
        select 
        'fm' || '_' || md5('GREGoR_R03_HMB_20250612' || '|' || cast(coalesce(participant.participant_id, '') as text))::text as "family_member",
        ...
        'fm' || '_' || md5('GREGoR_R03_HMB_20250612' || '|' || cast(coalesce(participant.participant_id, '') as text))::text as "id",
       'fm' || '_' || md5('GREGoR_R03_HMB_20250612' || '|' || cast(coalesce(participant.family_id, '') as text))::text as "family_id"
