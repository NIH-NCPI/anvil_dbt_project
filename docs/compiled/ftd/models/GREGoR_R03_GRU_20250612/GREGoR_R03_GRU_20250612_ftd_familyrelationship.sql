    ...
    with source as (
       ...
       CASE 
        ...
        SELECT 
        'sb' || '_' || md5('GREGoR_R03_GRU_20250612' || '|' || cast(coalesce(participant.participant_id, '') as text))::text as "other_family_member", -- not proband (niece or nephew's ID)
        (   select DISTINCT CAST(participant_id as STRING) FROM "dbt"."main_main"."GREGoR_R03_GRU_20250612_stg_participant" as stg 
    ...
    select DISTINCT
        'fm' || '_' || md5('GREGoR_R03_GRU_20250612' || '|' || cast(coalesce(participant_id, '') as text) || '|' || 'GREGoR_R03_GRU_20250612' || '|' || cast(coalesce(source.other_family_member, '') as text))::text as "id",
        relationship_code, 
        'sb' || '_' || md5('GREGoR_R03_GRU_20250612' || '|' || cast(coalesce(source.participant_id, '') as text))::text AS "family_member",
        'sb' || '_' || md5('GREGoR_R03_GRU_20250612' || '|' || cast(coalesce(source.other_family_member, '') as text))::text as "other_family_member"
    from source