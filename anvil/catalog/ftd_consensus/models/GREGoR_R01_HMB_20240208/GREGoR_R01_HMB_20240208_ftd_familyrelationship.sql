{{ config(materialized='table', schema='GREGoR_R01_HMB_20240208_data') }}
    ...
    with source as (
        ...
        SELECT
        (select DISTINCT CAST(participant_id as STRING) FROM {{ ref('GREGoR_R01_HMB_20240208_stg_participant') }} as stg 
       ...
       CASE 
        ...
        SELECT 
        {{ generate_global_id(prefix='sb',descriptor=['participant.participant_id'], study_id='GREGoR_R01_HMB_20240208') }}::text as "other_family_member", -- not proband (niece or nephew's ID)
        (   select DISTINCT CAST(participant_id as STRING) FROM {{ ref('GREGoR_R01_HMB_20240208_stg_participant') }} as stg 
            WHERE CAST(stg.family_id as STRING) = CAST(participant.family_id as STRING) 
    ...
    select DISTINCT
        ...
        relationship_code, 
        {{ generate_global_id(prefix='sb',descriptor=['source.participant_id'], study_id='GREGoR_R01_HMB_20240208') }}::text AS "family_member",
