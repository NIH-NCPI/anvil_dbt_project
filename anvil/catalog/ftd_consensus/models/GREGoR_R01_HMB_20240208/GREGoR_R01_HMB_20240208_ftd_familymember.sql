{{ config(materialized='table', schema='GREGoR_R01_HMB_20240208_data') }}
    ...
    with source as (
        select 
        {# {{ generate_global_id(prefix='',descriptor=[''], study_id='GREGoR_R01_HMB_20240208') }}::text as "family_member",#}
        ...
        {{ generate_global_id(prefix='fm',descriptor=['participant.participant_id'], study_id='GREGoR_R01_HMB_20240208') }}::text as "id",
       {{ generate_global_id(prefix='fm',descriptor=['participant.family_id'], study_id='GREGoR_R01_HMB_20240208') }}::text as "family_id"
