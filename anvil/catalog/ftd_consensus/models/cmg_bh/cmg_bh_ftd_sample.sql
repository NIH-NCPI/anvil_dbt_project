...
from (select distinct sample_id, subject_id, sample_source, sam.ingest_provenance
    from (
        select distinct subject_id, replace(ingest_provenance,'sample_','') as access, ingest_provenance, sample_source, sample_id from {{ ref('cmg_bh_stg_sample') }}) as sam
    left join (
        select distinct subject_id, replace(ingest_provenance,'subject_','') as access, ingest_provenance from {{ ref('cmg_bh_stg_subject') }}) as sub
