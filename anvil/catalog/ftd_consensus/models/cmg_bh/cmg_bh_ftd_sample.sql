...
select 
  ...
  {{ generate_global_id(prefix='sm',descriptor=['sample_id'], study_id='cmg_bh') }}::text as "id",
  {{ generate_global_id(prefix='sb',descriptor=['subject_id'], study_id='cmg_bh') }}::text as "subject_id",
  {{ generate_global_id(prefix='bc',descriptor=['sample_id'], study_id='cmg_bh') }}::text as "biospecimen_collection_id"  
