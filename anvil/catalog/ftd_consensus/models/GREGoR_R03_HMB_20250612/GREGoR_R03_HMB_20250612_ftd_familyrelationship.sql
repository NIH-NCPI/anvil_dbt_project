{{ config(materialized='table', schema='GREGoR_R03_HMB_20250612_data') }}
    ...
    with source as (
       ...
       CASE 
        ...
        SELECT 
        {{ generate_global_id(prefix='sb',descriptor=['participant.participant_id'], study_id='GREGoR_R03_HMB_20250612') }}::text as "other_family_member", -- not proband (niece or nephew's ID)
        (   select DISTINCT CAST(participant_id as STRING) FROM {{ ref('GREGoR_R03_HMB_20250612_stg_participant') }} as stg 
            WHERE CAST(stg.family_id as STRING) = CAST(participant.family_id as STRING) 
    ...
    select DISTINCT
        {{ generate_global_id(prefix='fm',descriptor=['participant_id','source.other_family_member'],study_id='GREGoR_R03_HMB_20250612') }}::text as "id",
        relationship_code, 
        {{ generate_global_id(prefix='sb',descriptor=['source.participant_id'], study_id='GREGoR_R03_HMB_20250612') }}::text AS "family_member",
        {{ generate_global_id(prefix='sb',descriptor=['source.other_family_member'], study_id='GREGoR_R03_HMB_20250612') }}::text as "other_family_member"
    from source
