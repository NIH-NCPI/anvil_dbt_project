{{ config(materialized='table') }}
    ...
    with source as (
       ...
       "twin_id"::text AS "twin_id"
        from {{ source('gregor_ts','GREGoR_synthetic_participant_revised07Aug2024') }}
    )

    select 
        *,
        concat(anvil_gregor_gss_u07_gru_participant_id, '-', anvil_gregor_gss_u07_gru_participant_id) as ftd_key
    from source
